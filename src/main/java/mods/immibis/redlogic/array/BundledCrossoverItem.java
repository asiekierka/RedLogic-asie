...
public class BundledCrossoverItem extends ItemBlock {
	public BundledCrossoverItem(Block block) {
		super(block);
		setHasSubtypes(true);
	}
	...
	public String getUnlocalizedName(ItemStack is) {
		return "tile.redlogic.bundled_crossover";
	}
	...
	public boolean canMergeIntoMicroblockContainer(World w, int x, int y, int z, int side, ItemStack stack) {
		...
		if(mbcb != null) {
			...
			if(block == mbcb) {
				BundledCrossoverTile testTile = new BundledCrossoverTile();
				testTile.init(side, side ^ 2);
				for(Part p : ((IMicroblockSupporterTile)w.getTileEntity(x, y, z)).getCoverSystem().getAllParts())
					if(testTile.isPlacementBlocked(p.type, p.pos))
						return false;
				return true;
			}
		}
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		if(!BundledCrossoverTile.checkCanStay(world, x, y, z, side^1))
			return false;
		...
		if(mbcb != null && world.getBlock(x, y, z) == mbcb) {
		...
		if(world.getBlock(x,y,z) instanceof BundledCrossoverBlock) {
			BundledCrossoverTile te = (BundledCrossoverTile)world.getTileEntity(x, y, z);
			te.init(side ^ 1, front);
			for(Part p : parts)
