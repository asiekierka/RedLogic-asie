...
public class GateBundledAND {
				...
				public ICompilableExpression[] compile(ICompileContext ctx, final ICompilableExpression[] inputs) {
					...
					for(int out = 0, in = 0; out < IPB; out++, in += numInputBundles) {
						if(numInputBundles == 0)
							rv[out] = MergeExpr.createAND();
						else if(numInputBundles == 1)
							rv[out] = MergeExpr.createAND(inputs[in]);
						else if(numInputBundles == 2)
							rv[out] = MergeExpr.createAND(inputs[in], inputs[in+1]);
						else if(numInputBundles == 3)
							rv[out] = MergeExpr.createAND(inputs[in], inputs[in+1], inputs[in+2]);
						else
	...
	public static class Logic extends GateLogic implements Stateless, WithBundledConnections {
		...
		public void update(short[] inputs, short[] outputs, int gateSettings) {
			int left = (gateSettings & 1) != 0 ? -1 : inputs[LEFT];
			int back = (gateSettings & 2) != 0 ? -1 : inputs[BACK];
			int right = (gateSettings & 4) != 0 ? -1 : inputs[RIGHT];
		...
		public boolean connectsToDirection(int side, int gateSettings) {
			switch(side) {
			case LEFT: return (gateSettings & 1) == 0;
			case BACK: return (gateSettings & 2) == 0;
			case RIGHT: return (gateSettings & 4) == 0;
			default: return true;
			}
		}
